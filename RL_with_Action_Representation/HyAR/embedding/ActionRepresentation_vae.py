...
class Action_representation(NeuralNet):
    ...
    def z_range(self, z, batch_size=100, range_rate=5):
        ...
        z = np.array(z)  # Convert to NumPy array for efficiency
        border = int(range_rate * (batch_size / 100))

        sorted_features = np.sort(z, axis=0)  # Sort each feature column independently

        lower_bounds = sorted_features[border]
        upper_bounds = sorted_features[-border - 1]
